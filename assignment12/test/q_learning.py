...
def get_action_index(readout_t, epsilon, t):
    ...
    if t < OBSERVE:
        ...
        rand_helper = random.random()
        if rand_helper < epsilon:
            action_index = random.randint(0, 2)
        else:
            # action_index = np.argmax(readout_t, axis=1)
            maxQ = max(readout_t)
            for i in range(len(readout_t)):
                if readout_t[i] == maxQ:
                    action_index = i
    return action_index
...
def run_selected_action(a_t, s_t, game_state):
    ...
    """
    image_data, reward, terminal = game_state.frame_step(a_t)
    s_t[:,:,0:3] = s_t[:,:,1:4]
    # s_t1 = np.stack((s_t, image_data))
    s_t[:,:,3] = image_data
    s_t1 = s_t
...
def compute_target_q(r_batch, readout_j1_batch, terminal_batch):
    ...
    for i in range(0, len(terminal_batch)):
        ...
        if terminal_batch[i]:
            target_q_value = r_batch[i]
        else:
            target_q_value = r_batch[i] + GAMMA * readout_j1_batch[i]
        target_q_batch.append(target_q_value)
